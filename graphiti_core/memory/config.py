...
def render_agent_instructions(root: Path) -> str:
    ...
    return '\n'.join(
        [
            ...
            '',
            'Preferred interface:',
            '- If the local Graphiti MCP server is available, use `graphiti mcp --transport stdio` and call the `recall`, `remember`, `index`, and `doctor` tools through MCP.',
            '- If MCP is not available, use the CLI commands directly.',
            '',
            'CLI workflow:',
