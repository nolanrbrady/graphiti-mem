...
def render_agent_instructions(root: Path) -> str:
    ...
    return '\n'.join(
        [
            ...
            '',
            'Preferred interface:',
            '- If the local Graphiti MCP server is available, use `graphiti mcp --transport stdio` and call `init_project`, `discover_history`, `list_bootstrap_artifacts`, `semantic_bootstrap`, `bootstrap_history`, `list_history_sessions`, `read_history_session`, `apply_agents_instructions`, `store_memory`, `recall_memory`, `index_project`, and `doctor` through MCP.',
            ...
            '- If approved, call `semantic_bootstrap` or `bootstrap_history`, then inspect `list_bootstrap_artifacts` for pending artifact distillation work before broad exploration.',
            ...
            '- During semantic bootstrap, call `list_bootstrap_artifacts`, read the listed high-signal files directly, write artifact-derived durable memories, and ensure those same files are indexed.',
            '- For artifact bootstrap memories, include exact file paths, commands, and subsystem names in the summary/details, set `artifact_path`, and pass provenance with `Source Mode: artifact_bootstrap` plus the source path and artifact type.',
            ...
            '- If bootstrap is pending, ask the user before running `graphiti bootstrap` or `graphiti bootstrap --agent codex`.',
            '',
            ...
            '- Use `graphiti bootstrap` for direct local bootstrap, or `graphiti bootstrap --agent codex` for an explicit Codex handoff after the user approves semantic bootstrap.',
            '',
            'Allowed kinds:',
            '- decision',
            '- constraint',
