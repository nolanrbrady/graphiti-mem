...
def parse_db_date(input_date: object | str | None) -> datetime | None:
    if neo4j_time is not None and isinstance(input_date, neo4j_time.DateTime):
...
def get_default_group_id(provider: GraphProvider) -> str:
    """Return the internal default group id for supported backends."""
    return ''
